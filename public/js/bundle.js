...
function tsvObj(sheetlink) {
    var rows = _axios2.default.get(sheetlink).then(function (res) {
    ...
    rowsJson.forEach(function (row) {
        if (row.subtype === 'background' && row.active) {
            ...
            obj = {
                ...
                background: row.src,
                title: row.coretitle,
                additions: [] //clear and start again
        ...
        if (row.subtype === 'pane' && row.active) {
            ...
            } else if (obj.additions.length === 0) {
                // current additions
...
function swap(file, link, rows) {
    var rowsRev = rows.map(function (row) {
    ...
    function update(a, b) {
        ...
        if (current.additions.length > 0 && internalIndex <= current.additions.length - 1) {
            ...
            if (subslide.tooltip) {
                ...
                var showHide = function showHide(type) {
                    if (type) {
                ...
                curTitle.onmouseover = function (e) {
                    return showHide('show');
                };
                curTitle.onmouseout = function (e) {
                    return showHide();
                };
                curPanel.onmouseover = function (e) {
                    return showHide('show');
                };
                curPanel.onmouseout = function (e) {
                    return showHide();
                };
...
function initClient() {
  var allFiles = gapi.client.init({
    ...
    handleAuthClick();
    (0, _publicTempReader.tsvObj)(sheetlink).then(function (res) {
      listFiles(res);
    });
  }).catch(console.log);
...
function listFiles(objComp) {
  var filesFound = gapi.client.drive.files.list({
    ...
    console.log(objComp, (0, _publicTempReader.sort)(objComp));
    //
    ...
    if (files && files.length > 0) {
      for (var i = 0; i < files.length; i++) {
        ...
        if (file.fileExtension === 'jpg' || file.fileExtension === 'png' || file.fileExtension === 'svg') {
          ...
          objUpdated = (0, _publicTempReader.swap)(file, link, objUpdated);
        }
      }
      var sortedItems = (0, _publicTempReader.sort)(objUpdated);
      (0, _publicTempReader.display)(sortedItems);
    } else {
